...
import {
  getMetadataDetail,
  searchMetadata,
} from "@main/services/metadataLookup";
...
export const registerLibraryIpc = (): void => {
  ...
  ipcMain.handle("library:getTracks", () => {
    try {
      ...
      const data = all.filter(
        (track) => !track.path || !containerPaths.has(track.path),
      );
  ...
  ipcMain.handle(
    "library:prefetchArtistImages",
    async (event, artistNames: string[]) => {
      if (!Array.isArray(artistNames)) return { success: true, data: {} };
      const data = await prefetchArtistImages(
        artistNames.slice(0, 500),
        (artistName, image) => {
  ...
  ipcMain.handle(
    "library:writeTags",
    async (_event, edits: TagEditRequest[]) => {
      try {
          ...
          if (coverSource) {
            if (!coverCache.has(coverSource)) {
              const loaded = coverPath
                ...
                : ((await fetchBytes(coverUrl!, { requireImage: true })) ??
                  undefined);
              coverCache.set(coverSource, loaded);
              if (!loaded && coverUrl) {
                libraryLog.warn(
                  `Cover download failed; writing text only: ${coverUrl}`,
                );
              }
            }
        ...
        const results = await getEngine().writeTrackTags(
          requests,
          getCoverCacheDir(),
        );
        ...
        libraryLog.info(
          `Tag write complete: ${upserts.length}/${results.length} succeeded`,
        );
        return { success: true, data };
      } catch (error) {
  ...
  ipcMain.handle("library:pickCoverImage", async () => {
    ...
    try {
      ...
      // 上限 15MB（FLAC 规范单 block 上限 16MB）
      if (stat.size > 15 * 1024 * 1024) {
        return { success: false, error: ErrorCode.COVER_TOO_LARGE };
      }
      // 预览只回小缩略图
  ...
  ipcMain.handle(
    "library:searchMetadata",
    async (_event, query: MetadataSearchQuery) => {
      try {
