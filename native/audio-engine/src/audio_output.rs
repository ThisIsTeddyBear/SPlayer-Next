...
mod pipewire_props {
    ...
    impl Guard {
        pub(super) fn set_stream_props(sample_rate: u32) -> Self {
            ...
            // Under Linux PipeWire, cpal streams do not carry node.rate or stable application metadata.
            // Injecting node.rate drives hardware DAC clock switching, while application.id and
            // fixed media.name allow WirePlumber to reliably persist application volume.
            unsafe {
